...
- `aiScore: 0` means "no evaluation row", not zero; on the wire an absence is an absence and nothing
  ...
  failure stores — so neither is judged, and both **share the field's MEDIAN position**: with no
  `GEMINI_API_KEY` every snap of the day is that fallback, and any order over them makes upload time
  half the score. `aiNorm` is that POSITION, never a rating — first place is exactly `HALF_WEIGHT`,
  and printing it under a bare "AI" is the whole of #97.
