...
- `eventStageKey` is every stage the DO PUBLISHES as one value — the phase, the podium's rank, its
  hand-over and the spin — and **null is "no event"**, which is what keeps the END of one from
  reading as a transition the screens have to react to. The beast is a clock moment inside the
  wheel, published with it, so it moves no key of its own and needs none.
